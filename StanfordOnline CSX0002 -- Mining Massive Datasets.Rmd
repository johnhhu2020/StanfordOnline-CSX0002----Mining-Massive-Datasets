...
*the smallest value is very small, and we can ignore the probability of a collision.*[][define "very small"]
So here's the algorithm in a nutshell.
[][The outer loop is on the rows.]**interesting, different with the book I suppose**
Okay.
[][For each row r, the first thing we do is compute each of the,]
[][perhaps, hundred hash values, h sub i of r.]
That's this.
[][Then, we'll loop over all the column c, and]**read the book and think**
[][if column c does not have a one in row r then we do nothing for r and c.]
Okay, but, now [][suppose matrix m has one in row r in column c.]**what is the matrix m, the original one**
[][Then we're going to loop over the index I.]*run the code in the brain and read the book*
...
We take M of i and c to be infinity initially.**SIG(i,c) matrix I suppose, M is the middle matrix**
So the first row in which we find has a won in column,
Also note that it is important we compute h survive r only once for
each hash function in each row.**because there will be only one 1 in each column, and we are searching 1s row by row**[][Think Here]
Outside the loop over the columns, that's, that was, that was this.

So, let's do a little example.[][Do check the sample on the book too, and also read this one, and think diff==========]
...
**That, in effect, gives us one column of the matrix.**[][so the column is actually the shingles extracked from the document?]
[][If so, we need to do one preliminary step, sort the data, so it is organized by row.]**Why, because the sequence in language**
That's not hard.
Start with a list of row column pairs where the ones are.
Initially sort it by column, **and sort these pairs by row.**[][what pairs???]


[][just go to read the book, there will be too much note to take, just go to read the book and watch the lecture]
...
sets of shingles to short signatures, consistently a vectors of integers.
...
For each selected hash function, we hash columns to buckets.
...
then they would be 2 to the 5 times 32, or 2 to the 160th power of buckets.
...
Put another way, the probability of a false negative,
...
the 20 bands is surely no more than 20 times that, or 20%.
that's, that's not great.
It means that among 40% similar underlying sets,
there are 20% false positives, pairs of signatures we will have to compare and
...
For example, for 20% Jaccard similarity, we get less than 1% false positives.
...
However, if the underlying Jaccard similarity exceeds 2,
...
it does leave a lot of false positives and negatives.
That is, for a given threshold t, all of these are false positives and
...
Okay?
As b and r get large, this function increasingly resembles a step function.
